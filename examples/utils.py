...
class SpotLessModule(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.mlp(x)
class BayesModule(torch.nn.Module):
    """robust Bayes MLP predictor class."""
...
def create_window(window_size, channel):
    ...
    return window
def ssimv1(img1, img2, window_size=11, size_average=True,womean=False):
    channel = img1.size(-3)
    ...
    return 1-_ssim(img1, img2, window, window_size, channel, size_average,womean)

def ssimv2(img1, img2, window_size=11, size_average=True,womean=False):
    channel = img1.size(-3)
    window = create_window(window_size, channel)
    ...
    return _ssimv2(img1, img2, window, window_size, channel, size_average,womean)

def _ssimv2(img1, img2, window, window_size, channel, size_average=True,womean=False):
    ...
    epsilonc = torch.ones_like(sigma1_sq)*epsilon
    sigma1_sq = torch.maximum(epsilonc, sigma1_sq)
    sigma2_sq = torch.maximum(epsilonc, sigma2_sq)
...
def _ssim(img1, img2, window, window_size, channel, size_average=True,womean=False):
    mu1 = F.conv2d(img1, window, padding=window_size // 2, groups=channel)
    mu2 = F.conv2d(img2, window, padding=window_size // 2, groups=channel)
